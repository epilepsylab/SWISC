...
def signal_length_test(decimated_channel):
    ...
    if channel_length > config.samples_expected:
        ...
        log_console.log_message(f'Channel truncated by {excess_samples} samples.',config.log_file_path)
        ...
        log_console.log_message(
            ...
            f'actual length: {channel_length}. Length rounded to nearest epoch.', config.log_file_path
        )
...
def invalid_data_test(decimated_channel):
    ...
    if np.any(np.isnan(decimated_channel))==True:
        ...
        log_console.log_message(
            f'Nan value located in ',config.log_file_path)
        return False
...
def run_all_tests(dec_data):
       ...
       for key in dec_data.keys():
            ...
            if correct_length==False or valid_data==False:
                log_console.log_message(f'Unit test error in {file_name}',config.log_file_path)
                return False
